 ...
 */
  ...
  for (let i = 0; i < sortedStations.length; i++) {
    ...
    changePreselectBikeToClickedStation(newDiv, sortedStations[i], tableID);
  }
...
function preselectStation(stations, availabilityKey, tableID) {
  console.log(availabilityKey, tableID);
  var index;

  var isInitialPick = availabilityKey == "bikes" || availabilityKey == "bike_stands";

  // If this is the first bike being loaded, an availability key will be included
  if (isInitialPick) {
    var selectedStationInfo = selectClosestStation(stations, availabilityKey);
    var availableStation = selectedStationInfo.availableStation;
    index = selectedStationInfo.index;
  }
  // Else the only station being passed in is the one to show in the preselect
  else {
    var availableStation = stations;
    index = Number(availabilityKey);
  }

  // If an available station exists show it
  if (availableStation) {
    const walkingTimeInMinutes = distanceToMinutes(availableStation.distance);
    generatePreSelectHTML(availableStation, index, tableID, walkingTimeInMinutes);
    updatePreSelectRouteParams(availableStation, tableID, walkingTimeInMinutes);
  }
...
function updatePreSelectRouteParams(availableStation, tableID, walkingTimeInMinutes) {
  if (tableID == "start") {
...
function generatePreSelectHTML(availableStation, index, tableID, walkingTimeInMinutes) {
  if (availableStation) {
...
function selectClosestStation(stations, availabilityKey) {
  for (let i = 0; i < stations.length; i++) {
    if (stations[i].station[availabilityKey] > 0) {
      return {availableStation: stations[i], index: i};
    }
  }
}
